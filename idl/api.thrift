...
struct douyin_favorite_action_request {
    1: string token     (api.form="token", api.vd="len($) > 0")  // 用户鉴权token
    2: i64 video_id     (api.form="video_id")// 视频id
    3: i32 action_type  (api.form="action_type")// 1-点赞，2-取消点赞
}
...
struct douyin_comment_action_request {
    1: string token        (api.form="token", api.vd="len($) > 0") // 用户鉴权token
    2: i64 video_id        (api.form="video_id")// 视频id
    3: i32 action_type     (api.form="action_type")// 1-发布评论，2-删除评论
    4: string comment_text (api.form="comment_text", api.vd="len($) > 0")// 用户填写的评论内容，在action_type=1的时候使用
    5: i64 comment_id      (api.form="comment_id")// 要删除的评论id，在action_type=2的时候使用
}
...
struct douyin_relation_action_request {
    1: string token       (api.form="token", api.vd="len($) > 0")// 用户鉴权token
    2: i64 to_user_id     (api.form="to_user_id")// 对方用户id
    3: i32 action_type    (api.form="action_type")// 1-关注，2-取消关注
}
